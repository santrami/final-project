...
io.on("connection", (socket) => {
  ...
  socket.on("give_room_tictactoe", () => {
    if (waitingSocketTTT === null || !waitingSocketTTT.connected)
      ...
      waitingSocketTTT
        .to(num)
        .emit("start_tictactoe", { room: num, player: "X", start });
      socket.to(num).emit("start_tictactoe", { room: num, player: "O", start });
      waitingSocketTTT = null;
    }
  });

  socket.on("send_restart_tictactoe", (room) =>
    io.to(room).emit("restart_tictactoe")
  );
  ...
  socket.on("s_next_round_rps", (obj) =>
    socket.to(obj.room).emit("next_round_rps")
  );
